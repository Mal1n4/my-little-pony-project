  ...
  <div class="main-content">
    <div class="col-4 d-flex card">
        <div class="flex-fill justify-content-center">
            <div class="name text-center p-4">
              <h3>{{ store.aradata[1].episode_name }} </h3>
              <h3>{{ store.aradata[1].episode }} </h3>
            </div>
            <div class="d-flex justify-content-center">
              <img class="photo" :src="store.aradata[1]['episode_img']" alt="Скриншот из серии" width="656px" height="369px">
            </div>
            <div class="episode-comment">
                <li v-for="comment in store.aradata[1].episode_comment" class="py-2">
                    {{ comment }}
                </li>
            </div>
        </div>
    ...
    <div class="fact">
      ...
      <div class="fact-text">
        ...
        <li v-for="item in store.aradata[1].fact" >
                    <h5>{{ item }}</h5>
        </li>
      </div>
    </div>
    ...
    <div class="col-4 d-flex card">
          <div class="flex-fill justify-content-center">
            <div class="name text-center p-4">
              <h3>{{ store.aradata[1].name }} </h3>
              <h3>{{ store.aradata[1].alias }} </h3>
            </div>
            <div class="d-flex justify-content-center">
              <img class="photo" :src="store.aradata[1]['img']" alt="Изображение готового рецепта" width="352px" height="340px">
            </div>
            <div>
              <span class="time d-flex"><img src="../assets/pictos/clocks.png" alt="Часы" width="42px" height="37px"><p class="w-auto">{{ store.aradata[1].time }}</p></span>
              <span class="ingredients d-flex"><img src="../assets/pictos/book.png" alt="Открытая книга" width="52px" height="59px"><div><li v-for="ingr in store.aradata[1].ingredients">
                {{ ingr }}
              </li></div></span>
              <span class="cooking d-flex"><img src="../assets/pictos/cook.png" alt="Венчик и поварской колпак" width="76px" height="49px"><div><li v-for="step in store.aradata[1].cooking" class="py-2">
                {{ step }}
              </li></div></span>
            </div>
            <span class="btns d-flex"><div class="saves"><img class="save" src="../assets/buttons/save.png" alt="Сохранить" width="173px" height="29px"><img @:click="store.statusUpdate(store.aradata[1]); buttonUpdate(store.aradata[1])" class="save_hover" src="../assets/buttons/save_hover.png" alt="Сохранить" width="173px" height="29px"></div>
...
<script setup>
  ...
  const buttonUpdate = (item) => {
    if(item.save_status){
    document.getElementsByClassName("save")[0].src = "/src/assets/buttons/saved.png"; //id рецепта должно совпадать с позицией на странице (т.е. с порядковым номером элемента класса на странице)
    document.getElementsByClassName("save_hover")[0].src = "/src/assets/buttons/delete.png";
    }
    else{
      document.getElementsByClassName("save")[0].src = "/src/assets/buttons/save.png";
      document.getElementsByClassName("save_hover")[0].src = "/src/assets/buttons/save_hover.png";
    }
  };
  ...
  onMounted(() => {
    buttonUpdate(store.aradata[recid]);
    }
  );
